...
apk upgrade
apk add jansson
# load module: ngx_http_auth_rbac_module.so
sed -i '/events {/i load_module "/usr/lib/nginx/modules/ngx_http_auth_rbac_module.so";' /etc/nginx/nginx.conf
EOS

COPY --from=builder /usr/lib/nginx/modules/ngx_http_auth_rbac_module.so /usr/lib/nginx/modules/ngx_http_auth_rbac_module.so
